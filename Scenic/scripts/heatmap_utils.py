    ...
    if split_repressor_activator and len(set(plotting_df["repressor_activator"])) == 2:
        ...
        if orientation == 'vertical':
            ...
            plot = (
                ...
                + theme(axis_title_x = element_blank(), axis_title_y = element_blank()))
    if save is not None:
        plot.save(save, limitsize = False)
    else:
        return plot
...
def my_heatmap_df(
    ...
    plotting_df = my_generate_dotplot_df(
...
def make_heatmap_df(scplus_mdata, celltype_col, motifAnno == "direct", onlypos == False):
    ...
    df = my_heatmap_df(
        scplus_mudata = scplus_mdata,
        color_modality = f"{motifAnno}_gene_based_AUC",
        size_modality = f"{motifAnno}_region_based_AUC",
        group_variable = f"scRNA_counts:{celltype_col}",
        eRegulon_metadata_key = f"{motifAnno}_e_regulon_metadata",
    ...
    if onlypos == True:
        df = df[df['eRegulon_name'].str.contains('+/+', regex = False)]
    ...
    rtk_sp = [r.split("_") for r in rss.columns]
    rtk_sub = [r[0] + "_" +  r[1] + "_" + r[2] for r in rtk_sp]
    rss.columns = rtk_sub
    rss_scaled = _scale(rss)

    sp_vec = list()
    sp_vec_scaled = list()
    for i in range(0, len(df)):
        ct = df[f"scRNA_counts:{celltype_col}"].iloc[i]
        reg = df["eRegulon_name"].iloc[i]

        sp_vec.append(rss[reg].loc[ct])
        sp_vec_scaled.append(rss_scaled[reg].loc[ct])

    df["specificity"] = sp_vec
    df["specificity_scaled"] = sp_vec_scaled

    ### Get TF averages
    rna_mdata = scplus_mdata["scRNA_counts"]

    # Get eRegulon list and extract TF. The rss table had list as cols
    regs = rss.columns
    TFS = [x.split("_")[0] for x in regs]

    # Get index of TF in assay
    gis = [rna_mdata.var_names.get_loc(TF) for TF in TFS]

    # Extract matrix of cell by TF
    spdf = pd.DataFrame.sparse.from_spmatrix(rna_mdata.X[:, gis]).transpose()

    spdf.index = TFS
    spdf.columns = rna_mdata.obs_names

    # Get vector of celltype identity per cell
    group_by = rna_mdata.obs[f"{celltype_col}"].tolist()
    spdf_t = spdf.transpose()

    # Group cells by celltype and take mean *******
    mat_avg = spdf_t.groupby(group_by).mean()
    mat_avg_scaled = _scale(mat_avg)

    mat_avg.columns = regs
    mat_avg_scaled.columns = regs

    # Transform dataframe into long format
    mat_avg_long = mat_avg.stack().reset_index()
    mat_avg_sc_long = mat_avg_scaled.stack().reset_index()

    mat_avg_sc_long.columns = [f"scRNA_counts:{celltype_col}", "eRegulon_name", "TF_avg_scaled"]
    mat_avg_long.columns = [f"scRNA_counts:{celltype_col}", "eRegulon_name", "TF_avg"]


    dff = df.merge(
            mat_avg_sc_long,
            on = [f"scRNA_counts:{celltype_col}", "eRegulon_name"])
...
def heatmap_plotting(scplus_mdata, dff, motifAnno = "direct", celltype_col, outdir):
    my_heatmap_dotplot(
            scplus_mudata = scplus_mdata,
            plotting_df = dff,
            color_modality = "TF_avg_scaled",
            size_modality = "specificity_scaled",
            group_variable = f"scRNA_counts:{celltype_col}",
            eRegulon_metadata_key = f"{motifAnno}_e_regulon_metadata",
            color_feature_key = "Gene_signature_name",
            size_feature_key = "Region_signature_name",
